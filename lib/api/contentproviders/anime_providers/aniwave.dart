  ...
  Future<List<StreamClass>> getStream() async {
    try {
      ...
      if (episode.isEmpty) {
        ...
        final infoUrl = document
            .querySelector("div.group div.relative a")
            ?.attributes['href'];
        if (infoUrl == null)
        ...
        final container = body
            ...
            .where((e) => !episodeList
                .contains(e.split("ep_no")[1].split(",")[0].split(":")[1]))
            .map((e) {
          String currentEpisode =
              isNumeric(e.split("ep_no").last.split(",").first.split(":").last)
                  ? e.split("ep_no")[1].split(",")[0].split(":")[1]
                  : (double.parse(previousEpisode) - 1).toString();
          final data = {
            "episodeId": e.split("ep_no")[0].split(":")[1].split(",")[0],
            "episodeNumber": currentEpisode,
            "watchUrl": watchUrl.split("ep-")[0] +
                "ep-${e.split("ep_no")[0].split(":")[1].split(",")[0]}"
          };
