...
class Embed {
  ...
  Future<List<StreamClass>> getStream() async {
    try {
      ...
      firstDecodeParts = firstDecodeParts.map((item) => item.split("").reversed.join("")).toList();
      String secondDecodedString = await stringAtob(firstDecodeParts.join("").split("").reversed.join(""));
  ...
  Future<List<StreamClass>> getAllStreams(data) async {
    final streams = (data as List).where((item)=>item['name']=='viper').map((item) async {
      ...
      final sourceUrl = data['source'].toString().replaceAll("embed.su/api/proxy/viper/", "");
  ...
  String _resolveStreamUrl(String streamUrl, String baseUrl) {
    final resolvedUri = streamUrl.replaceAll("/api/proxy/viper/","https://").replaceAll(".png",".m3u8");
