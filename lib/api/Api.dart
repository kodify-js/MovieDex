...
class Api {
            ...
            try {
                ...
                final List<Contentclass> result = (response['results'] as List)
                    ...
                    .map((movie) {
                        return Contentclass(
                            id: movie['id'], 
                            backdrop: imagePath(size: imageSize??ImageSize.original, path: movie['backdrop_path']), 
    ...
    Future<List<Contentclass>> getTrending({required type, required language}) async {
        ...
        return _fetchWithCache<List<Contentclass>>(endpoint, () async {
            try {
                final data = await http.get(Uri.parse('$baseUrl/3/trending/$type/day?api_key=$apiKey&language=en-US'));
                final response = jsonDecode(data.body);
